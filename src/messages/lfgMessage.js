 ...
 * @param openInvitation
 */
function LFGMessage(game, links, requestor, taggables, openInvitation = false) {

    const titleComponent = new TextDisplayBuilder().setContent(`## <@${requestor}> is looking for players to play **${game.name}**`);
    ...
    const linksContent = links.map((link, index) => { return `- [${link.name}](<${link.url}>)`; }).join(' \n');
    const linksComponent = new TextDisplayBuilder().setContent(`**Available on:** \n ${linksContent}`);

    const availableSectionComponent = new SectionBuilder()
