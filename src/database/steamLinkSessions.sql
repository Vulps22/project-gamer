...
ALTER TABLE `steam_link_sessions`
  ...
  ALTER COLUMN `token` SET COMMENT 'Unique session token for Steam OAuth flow.',
  ALTER COLUMN `userId` SET COMMENT 'Foreign key linking to the Discord user initiating the Steam link.',
  ALTER COLUMN `expiresAt` SET COMMENT 'Session expiration timestamp (15 minutes from creation).',
  ALTER COLUMN `createdAt` SET COMMENT 'Timestamp when the session was created.';
